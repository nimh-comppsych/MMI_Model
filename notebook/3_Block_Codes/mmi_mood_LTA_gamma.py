...
#             avg_reward_trial = reward_sum/trial_num
#             trials.at[trials.index[i], 'LTA']=avg_reward_trial
...
#             #import pdb; pdb.set_trace()
    ...
    ####### Log_Like NaN value trace #####
            if np.isnan(trials.at[trials.index[i], 'log_like']):
#                 print((GAMMA, BETA, W, A, T0, P0, LAMBDA, WLTA, WRPE, B, Sv), flush=True)
#                 print('RT,v:',(Reac_time,v_mean), flush=True)
#                 print('EG,EC,RPE:',(EC,EG,RPE), flush=True)
                #print(Trange_nsamp, flush=True)
                #print('w parameter line 256(proc_trials):',W,flush=True)
...
def load_mmi_data(filename):
    ...
    dat = dat.loc[~np.isnan(dat.AmountChange)]
    
    ### reseting the indices of the rows starting from 0
    ...
    dat['Mood'] = dat['TrialHappiness__SliderResp']

    # add a choice
    dat['Choice'] = 'None'
    ind = (dat['RT']>0)
    dat.loc[ind&(dat['Outcome']=='Certain'), 'Choice'] = 'Certain'
    dat.loc[ind&(dat['Outcome']=='Outcome1'), 'Choice'] = 'Gamble'
    dat.loc[ind&(dat['Outcome']=='Outcome2'), 'Choice'] = 'Gamble'

    # add outcome amount
    dat['OutcomeAmount'] = dat['CertainAmount']
    ind = (dat['Outcome']=='Outcome1')
    dat.loc[ind, 'OutcomeAmount'] = dat.loc[ind, 'Outcome1Amount']
    ind = (dat['Outcome']=='Outcome2')
    dat.loc[ind, 'OutcomeAmount'] = dat.loc[ind, 'Outcome2Amount']

    # add greater and lesser amounts
    dat['GreaterAmount'] = dat['Outcome1Amount']
    dat['LesserAmount'] = dat['Outcome2Amount']
