...
class MMIModel():
    ...
    def update_p(self, trial):
        if trial.Choice == 'Gamble':
            RPE = self._apply_gamma(trial['OutcomeAmount']) - trial['EG']
            I = float(trial['Won'])
    ...
    def calc_latents(self, trial, latents):
        ...
        if trial.Choice == 'Gamble':
            latents2['EG_sum'] = latents2['EG_sum']+0.5*(trial['GreaterAmount']+trial['LesserAmount'])
                                   
             
            latents2['RPE_sum'] = latents2['RPE_sum']+trial['OutcomeAmount']-0.5*(trial['GreaterAmount']+trial['LesserAmount'])
        else:
            latents2['CA_sum'] = latents2['CA_sum'] + trial['CertainAmount']
        return latents2
    ...
    def calc_trial_like(self, trial, save_post=False):
        ...
        if not np.isnan(trial['Mood']):
            ...
            pred_mood = self.params['b'] + \
                        self.params['w_CA'] * trial['CA_sum'] + \
                        self.params['w_EG'] * trial['EG_sum'] + \
    ...
    def proc_trials(self, trials, save_posts=False):
        ...
        for i in range(len(trials)):
            ...
            trials.at[trials.index[i], 'EG_sum'] = latents['EG_sum']
